(function (undefined) {
    ...
    function getIPAServer() {
        return ke.ext.util.storageUtil.getVal('user_country') === 'cn'
            ? 'asia.gikken.co/mateapi'
            : 'api' + ke.ext.util.storageUtil.getVal('misc_server') + '.matetranslate.com';
    }
    ...
    pl.extend(ke.ext.googleApi, {
        ...
        getWordsInfo: function (words, fn) {
            ...
            var xhr = $.ajax({
                url: 'https://api' + ke.ext.util.storageUtil.getVal('misc_server') + '.matetranslate.com/v3/get_words_info',
                type: 'POST',
                dataType: 'json',
                data: JSON.stringify({
                    words: words
        ...
        getInternalJSONFormat: function (output, original) {
            ...
            var translations = [
                ...
                '',     // 2 - ipa
                '',     // 3 - translation
                '',     // 4 - transcription for translation
                '',     // 5 - from lang
                '',     // 6 - to lang
                [
                    [], // no category
