function DownloadsInterceptManager()
{
    ...
    /* @requestBodyByRequestId - simple map of requestId -> postData for interception in Firefox */
    this.requestBodyByRequestId = new Map;
    /* @requestBodyByRequestUrl - saves postData by url for interception in Chrome, because of requestId cannot be used */
    this.requestBodyByRequestUrl = [];
...
DownloadsInterceptManager.prototype.removeRequestBodyByOriginalUrl = function(
    url, time)
{
    var index = this.requestBodyByRequestUrl.findIndex(item => (item.time === time && item.url === url));
    if (index !== -1) {
        this.requestBodyByRequestUrl.splice(index, 1);
    }
};

DownloadsInterceptManager.prototype.returningRequestBodyIndexByOriginalUrl = function(
    url)
{
    var currentIndex = -1;
    for (var key in this.requestBodyByRequestUrl) {
        var item = this.requestBodyByRequestUrl[key];
        if (item.url === url && (currentIndex === -1 || item.time > this.requestBodyByRequestUrl[currentIndex].time)) {
            currentIndex = key;
        }
    }
    return currentIndex;
...
{
    var postData = '';
    var requestBodyIndex = this.returningRequestBodyIndexByOriginalUrl(
        downloadItem.finalUrl);
    if (requestBodyIndex !== -1)
    {
        postData = this.requestBodyByRequestUrl[requestBodyIndex].postData;
        this.requestBodyByRequestUrl.splice(requestBodyIndex, 1);
    }
    ...
    */
    ...
    chrome.tabs.query({active: true, currentWindow: true}, function(tabs) {
        ...
        if (this.inSkipList(active_tab_url, downloadItem.filename)) {
            suggest(downloadItem);
            return;
        }
        ...
        this.onDownloadIntercepted(new DownloadInfo(
            ...
            downloadItem.referrer, postData)
            // null,
...
DownloadsInterceptManager.prototype.onBeforeRequest = function(details)
{
    if (details.method == "POST")
    {
        ...
        this.requestBodyByRequestId.set(details.requestId, body);
        setTimeout(this.requestBodyByRequestId.delete.bind(this.requestBodyByRequestId, details.requestId), 120000);
       
        var currentTime = + new Date;
        this.requestBodyByRequestUrl.push({"url" : details.url, "postData" : body, "time": currentTime});
        setTimeout(this.removeRequestBodyByOriginalUrl.bind(this, details.url, currentTime), 120000);
    }
};
    ...
    details)
{
        ...
        if (file)
        {
            ...
            else
            {
                ...
                var downloadInfo = new DownloadInfo(
                    ...
                    "",
                    referrer,
                    this.requestBodyByRequestId.get(details.requestId));
    ...
    this.requestBodyByRequestId.delete(details.requestId);
